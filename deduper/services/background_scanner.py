    ...
    def _get_folder_mtime(self, folder_path: str) -> float:
        ...
        try:
            ...
            for entry in os.scandir(folder_path):
                try:
                    # Skip thumbnail files and cache files when checking for changes
                    # These are generated by the app and shouldn't trigger rescans
                    entry_name = entry.name
                    if entry_name.startswith("thumb") or entry_name == ".deduper" or entry_name == ".json":
                        continue
